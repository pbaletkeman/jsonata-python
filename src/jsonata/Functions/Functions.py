...
class Functions:
    ...
    def replace(
        ...
        if isinstance(pattern, str):
            ...
            if limit is None:
                ...
                result = string
                while count < limit:
                    result = Functions.safe_replace_first(
                        result, pattern, str(replacement)
                    )
                    count += 1
                return result
    ...
    def format_number(
        ...
        if all(not fmt for fmt in fmt_tokens):
            raise JException("both integer and fractional parts are empty", -1)
    ...
    def format_digits(
        ...
        for fmt_char in reversed(fmt):
            if fmt_char in digits_family or fmt_char == optional_digit:
                ...
                not result
                or not result[-1] in digits_family
                and grouping_separator
        ...
        while result and unicodedata.category(result[-1]) not in (
            "Nd",
            "Nl",
            "No",
            "Lu",
    ...
    def map(arr: Optional[Sequence], func: Any) -> Optional[list]:
        ...
        return Utils.create_sequence_from_iter(result)
    ...
    def each(obj: Optional[Mapping], func: Any) -> Optional[list]:
        ...
        result = (
            res
            for res in (
                Functions.func_apply(
                    func, Functions.hof_func_args(func, value, key, obj)
                )
    ...
    def type(value: Optional[Any]) -> Optional[str]:
        ...
        if Utils.is_function(value) or Functions.is_lambda(value):
            return "function"
    ...
    def sift(arg: Optional[Mapping], func: Any) -> Optional[dict]:
        ...
        result = {
            ...
            if Jsonata.boolize(
                Functions.func_apply(
                    func, Functions.hof_func_args(func, entry, item, arg)
                )
            )
        }
