...
def _roman_to_decimal_impl(roman: str):
    decimal = 0
    maxv = 1
    for i in range(len(roman) - 1, -1, -1):
        ch = roman[i]
        value = DateTimeUtils._roman_values[str(ch)]
        if value < maxv:
            decimal -= value
        else:
            maxv = value
...
@cython.cfunc
@cython.locals(top=cython.long, value=cython.long)
def _words_to_long_impl(text: str):
    
    parts = _re.split(",\\s|\\sand\\s|[\\s\\-]", text)
    values = [DateTimeUtils._word_values_long[str(part)] for part in parts]
    segs = []
    top = 0
    for value in values:
        if value < 1000:
...
class DateTimeUtils:
    ...
    def _lookup(num: int, prev: int, ordinal: bool) -> str:
        if num <= 19:
            ...
            words = ((", " if prev else "") + DateTimeUtils._lookup(mant, False, False) + " " + str(DateTimeUtils._magnitudes[mag - 1]))
            if remainder > 0:
                words += DateTimeUtils._lookup(remainder, True, ordinal)
            elif ordinal:
                words += "th"
    ...
    def _static_initializer():
        ...
        while i < len(DateTimeUtils._few):
            DateTimeUtils._word_values[str(DateTimeUtils._few[i]).casefold()] = i
            i += 1
        i = 0
        while i < len(DateTimeUtils._ordinals):
            DateTimeUtils._word_values[str(DateTimeUtils._ordinals[i]).casefold()] = i
            i += 1
        i = 0
        while i < len(DateTimeUtils._decades):
            lword = DateTimeUtils._decades[i].casefold()
            DateTimeUtils._word_values[str(lword)] = (i + 2) * 10
            DateTimeUtils._word_values[str(lword[0:len(lword) - 1]) + "ieth"] = DateTimeUtils._word_values[str(lword)]
            i += 1
        DateTimeUtils._word_values[str("hundredth")] = 100
        DateTimeUtils._word_values[str("hundreth")] = 100
        i = 0
        while i < len(DateTimeUtils._magnitudes):
            lword = DateTimeUtils._magnitudes[i].casefold()
            val = int(10 ** ((i + 1) * 3))
            DateTimeUtils._word_values[str(lword)] = val
            DateTimeUtils._word_values[str(lword + "th")] = val
            i += 1
        i = 0
        while i < len(DateTimeUtils._few):
            DateTimeUtils._word_values_long[str(DateTimeUtils._few[i]).casefold()] = int(i)
            i += 1
        i = 0
        while i < len(DateTimeUtils._ordinals):
            DateTimeUtils._word_values_long[str(DateTimeUtils._ordinals[i]).casefold()] = int(i)
            i += 1
        i = 0
        while i < len(DateTimeUtils._decades):
            lword = DateTimeUtils._decades[i].casefold()
            DateTimeUtils._word_values_long[str(lword)] = int((i + 2)) * 10
            DateTimeUtils._word_values_long[str(lword[0:len(lword) - 1]) + "ieth"] = DateTimeUtils._word_values_long[str(lword)]
            i += 1
        DateTimeUtils._word_values_long[str("hundredth")] = 100
        DateTimeUtils._word_values_long[str("hundreth")] = 100
        i = 0
        while i < len(DateTimeUtils._magnitudes):
            lword = DateTimeUtils._magnitudes[i].casefold()
            val = int(10 ** ((i + 1) * 3))
            DateTimeUtils._word_values_long[str(lword)] = val
            DateTimeUtils._word_values_long[str(lword + "th")] = val
            i += 1
    ...
    def words_to_number(text: str) -> int:
        ...
        values = [DateTimeUtils._word_values[str(part)] for i, part in enumerate(parts)]
        segs = deque()
        segs.append(0)
        for value in values:
            if value < 100:
    ...
    def words_to_long(text: str) -> int:
        parts = re.split(",\\s|\\sand\\s|[\\s\\-]", text)
        values = [DateTimeUtils._word_values_long[str(part)] for i, part in enumerate(parts)]
        segs = deque()
        segs.append(int(0))
        for value in values:
            if value < 100:
    ...
    def roman_to_decimal(roman: str) -> int:
        ...
        for i, digit in enumerate(reversed(roman)):
            value = DateTimeUtils._roman_values[str(digit)]
            if value < max:
                decimal -= value
            else:
                max = value
    ...
    def _format_integer(value: int, format: Optional[Format]) -> str:
        ...
        if format.primary == DateTimeUtils.Formats.LETTERS:
            formatted_integer = DateTimeUtils._decimal_to_letters(int(value), "A" if format.case_type == DateTimeUtils.TCase.UPPER else "a")
    ...
    def format_datetime(millis: int, picture: Optional[str], timezone: Optional[str]) -> str:
        ...
        if picture is None:
            if DateTimeUtils._iso8601_spec is None:
                DateTimeUtils._iso8601_spec = DateTimeUtils._analyse_datetime_picture("[Y0001]-[M01]-[D01]T[H01]:[m01]:[s01].[f001][Z01:01t]")
    ...
    @staticmethod
    def _format_component(date: datetime.datetime, marker_spec: SpecPart, offset_hours: int, offset_minutes: int) -> str:
    ...
    class YearMonth:
        ...
        def next_month(self):
            return DateTimeUtils.YearMonth(self.year + 1, 1) if self.month == 12 else DateTimeUtils.YearMonth(self.year, self.month + 1)

        def previous_month(self):
            return DateTimeUtils.YearMonth(self.year - 1, 12) if self.month == 1 else DateTimeUtils.YearMonth(self.year, self.month - 1)
