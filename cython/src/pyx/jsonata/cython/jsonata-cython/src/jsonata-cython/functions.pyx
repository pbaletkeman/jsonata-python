# cython: language_level=3str, boundscheck=False, wraparound=False, nonecheck=False, cdivision=True, infer_types=True
cimport cython
from libc.math cimport sqrt as c_sqrt, floor as c_floor, ceil as c_ceil, fabs as c_fabs, pow as c_pow
from libc.stdlib cimport rand, RAND_MAX, srand, malloc, free
from libc.time cimport time as c_time, time_t, gmtime, strftime, tm
...
class Functions:
    ...
    @staticmethod
    def _string(arg, prettify: bool) -> str:
    ...
    #     
    @staticmethod
    def substring_before(string, chars):
        # undefined inputs always return undefined
        if string is None:
            return None
    ...
    #     
    @staticmethod
    def substring_after(string, chars):
        # undefined inputs always return undefined
        if string is None:
            return None
    ...
    #     
    @staticmethod
    def pad(string, width, pad_char=None, **kwargs):
        # accept legacy keyword: char=...
        if pad_char is None and 'char' in kwargs:
            pad_char = kwargs['char']
    ...
    @staticmethod
    def left_pad(string, size, pad_str):
        if string is None:
            return None
    ...
    # Source: Jsonata4Java PadFunction
    @staticmethod
    def right_pad(string, size, pad_str):
        if string is None:
            return None
    ...
    #     
    @staticmethod
    def evaluate_matcher(matcher: re.Pattern, string) -> list[RegexpMatch]:
        res = []
        matches = matcher.finditer(string)
        for m in matches:
            groups = []
            # Collect the groups: tuple
            g = 1
    ...
    #     
    @staticmethod
    def contains(string, token: Union[None, str, re.Pattern]):
        # undefined inputs always return undefined
        if string is None:
            return None
    ...
    #     
    @staticmethod
    def match_(string, regex, limit) -> list[dict]:
        # undefined inputs always return undefined
        if string is None:
            return None
    ...
    #     
    @staticmethod
    def safe_replace_all(s: str, pattern: re.Pattern, replacement):
    ...
    #     
    @staticmethod
    def safe_replace_all_fn(s: str, pattern: re.Pattern, fn) -> str:
        def replace_fn(t):
    ...
    #     
    @staticmethod
    def safe_replace_first(s: str, pattern: re.Pattern, replacement: str):
        replacement = Functions.safe_replacement(replacement)
        r = None
        for i in range(0, 10):
            try:
    ...
    @staticmethod
    def replace(string, pattern: Union[str, re.Pattern], replacement, limit):
        if string is None:
            return None
    ...
    #     
    @staticmethod
    def decode_url_component(string):
        # undefined inputs always return undefined
        if string is None:
            return None
    ...
    @staticmethod
    def split(string, pattern: Union[str, Optional[re.Pattern]], limit):
        if string is None:
            return None
    ...
    @staticmethod
    def format_number(value, picture, decimal_format):
        ...
        if value >= 0:
            ...
            subpic = sub_pictures[len(sub_pictures) - 1]
        ...
        if exp_fmt and value:
            ...
            if abs(value) > 1:
                ...
                if not num_digits:
                    ...
                    elif not fmt_tokens[len(fmt_tokens) - 1]:
                        fmt_tokens[len(fmt_tokens) - 1] = zero_digit

            elif len(fmt_tokens) > 1 and fmt_tokens[len(fmt_tokens) - 1] and value >= 0:
                v = abs(value) * 10
                while v < 10 ** num_digits:
                    exp_value -= 1
                    v *= 10
        ...
        if len(fmt_tokens) == 1 or not fmt_tokens[len(fmt_tokens) - 1]:
            exp = decimal.Decimal('1')
        else:
            k = -1
            for ch in fmt_tokens[len(fmt_tokens) - 1]:
        ...
        if len(fmt_tokens) > 1 and fmt_tokens[len(fmt_tokens) - 1]:
            ...
            decimal_part = Functions.format_digits(chunks[1], fmt_tokens[len(fmt_tokens) - 1], **kwargs)

            for ch in reversed(fmt_tokens[len(fmt_tokens) - 1]):
                if ch == optional_digit:
                    if decimal_part and decimal_part[len(decimal_part) - 1] == zero_digit:
                        decimal_part = decimal_part[:len(decimal_part) - 1]
                else:
                    if not decimal_part:
    ...
    def format_digits(digits: str,
        ...
        for fmt_char in reversed(fmt):
            ...
            elif not result or not result[len(result) - 1] in digits_family and grouping_separator \
                    and result[len(result) - 1] != grouping_separator:
        ...
        if num_digit:
            ...
            if len(sep) != 1:
                ...
                if len(chunks[0]) > len(chunks[len(chunks) - 1]):
                    repeat = None
                elif last_idx >= 0 and all(len(item) == len(chunks[last_idx]) for item in chunks[1:last_idx]):
                    repeat = len(chunks[len(chunks) - 1]) + 1
                else:
                    repeat = None
        ...
        while result and \
                unicodedata.category(result[len(result) - 1]) not in ('Nd', 'Nl', 'No', 'Lu', 'Ll', 'Lt', 'Lm', 'Lo'):
            result.pop()
    ...
    @staticmethod
    def get_function_arity(func) -> int:
        from jsonata import jsonata
    ...
    #     
    @staticmethod
    def hof_func_args(func, arg1, arg2, arg3) -> list:
        func_args = [arg1]
    ...
    #     
    @staticmethod
    def fold_left(sequence, func, init):
        # undefined inputs always return undefined
        if sequence is None:
            return None
        result = None
    ...
    #     
    @staticmethod
    def assert_fn(condition, message) -> None:
    ...
    #     
    @staticmethod
    def lookup(input: Union[Mapping, Optional[Sequence]], key):
    ...
    #     
    @staticmethod
    def datetime_to_millis(timestamp, picture):
        # undefined inputs always return undefined
    ...
    #     
    @staticmethod
    def datetime_from_millis(millis, picture, timezone):
        # undefined inputs always return undefined
        if millis is None:
            return None
    ...
    #     
    @staticmethod
    def parse_integer(value, picture):
        if value is None:
            return None
